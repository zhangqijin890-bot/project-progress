...
/**
* 稳定、文件系统安全的项目 key —— 只由工作区路径生成，保证「一个工作区永远对应
* 一个项目目录」（会话标题只作为展示信息，不参与 key）。
*/
function projectKey(path) {
	const slug = String(basename(path) || "project")
		.toLowerCase()
		.replace(/[^a-z0-9\u4e00-\u9fff]+/g, "-")
		.replace(/^-+|-+$/g, "")
		.slice(0, 40) || "project";
...
async function openProject(projectsDir, session, { create = false } = {}) {
	...
	if (meta) {
		if (create) {
			...
			// 已有项目不覆盖标题，仅当标题缺失时补上当前会话标题。
			if (typeof meta.title !== "string" || meta.title.length === 0) meta.title = title;
		}
...
/**
* 迁移：把同一工作区（相同 project.json.path）的多个旧项目目录合并成一个。
* 旧版本用「会话标题+路径」生成 key，同一工作区不同标题会拆成多个目录；
* 现在统一按路径归并。幂等、非破坏性：目标目录里已有的内容也会参与合并。
* @returns 合并的项目数。
*/
async function migrateProjects(projectsDir, config) {
	let entries;
	try {
	...
	for (const entry of entries) {
		...
		const meta = await readJson(join(dir, "project.json"));
		if (meta === void 0 || typeof meta.path !== "string" || meta.path.length === 0) continue;
	...
	for (const [path, group] of byPath) {
		...
		// 目标目录：按新 key 计算；若已在组内则并入（组内其他目录删掉）。
		const targetDir = join(projectsDir, projectKey(path));
		const merged = await mergeProjectGroup(group, targetDir, config);
		if (!merged) continue;
		mergedCount += 1;
...
/** 合并一组同路径的项目目录到 targetDir（目标目录若存在也参与合并）。 */
async function mergeProjectGroup(group, targetDir, config) {
	try {
		const dirs = group.some((item) => item.dir === targetDir)
			? group.map((item) => item.dir)
			: [targetDir, ...group.map((item) => item.dir)];
		const metas = [];
		...
		for (const dir of dirs) {
			...
			metas.push(meta);
			for (const record of meta.sessions ?? []) {
				if (record?.id === void 0) continue;
				const existing = sessions.get(record.id);
				if (existing === void 0 || (record.turns ?? 0) > (existing.turns ?? 0)) sessions.set(record.id, record);
			}
			if (typeof meta.createdAt === "string" && (createdAt === void 0 || meta.createdAt < createdAt)) createdAt = meta.createdAt;
			if (typeof meta.updatedAt === "string" && (updatedAt === void 0 || meta.updatedAt > updatedAt)) updatedAt = meta.updatedAt;
			const entries = (await readJson(join(dir, "log.json"))) ?? [];
			for (const entry of entries) {
				const key = `${entry.sessionId}#${entry.turn}`;
				if (entry?.sessionId === void 0 || seenLog.has(key)) continue;
				seenLog.add(key);
				log.push(entry);
			}
			const noteList = (await readJson(join(dir, "notes.json"))) ?? [];
			for (const item of noteList) {
				const key = JSON.stringify({ time: item?.time, note: item?.note });
				if (seenNotes.has(key)) continue;
				seenNotes.add(key);
				notes.push(item);
			}
		}
		log.sort((a, b) => String(a.time).localeCompare(String(b.time)) || (a.turn ?? 0) - (b.turn ?? 0));
		notes.sort((a, b) => String(a.time).localeCompare(String(b.time)));
		// 摘要：从确实存在非空 digest.txt 的目录里挑更新时间最新的一个。
		const digestDirs = [];
		for (const dir of dirs) {
			const text = await readFile(join(dir, "digest.txt"), "utf8").catch(() => "");
			if (text.trim().length > 0) digestDirs.push(dir);
		}
		const dirUpdatedAt = new Map();
		for (const dir of dirs) {
			const meta = metas[dirs.indexOf(dir)];
			dirUpdatedAt.set(dir, meta?.updatedAt ?? "");
		}
		digestFrom = digestDirs.sort((a, b) => String(dirUpdatedAt.get(b)).localeCompare(String(dirUpdatedAt.get(a))))[0];
		const title = [...metas].sort((a, b) => String(b.updatedAt ?? "").localeCompare(String(a.updatedAt ?? "")))[0]?.title ?? basename(targetDir);
		const counters = {
			turns: [...sessions.values()].reduce((total, record) => total + (record.turns ?? 0), 0)
		};
		const mergedMeta = {
			version: 1,
			id: projectKey(metas[0]?.path ?? ""),
			title,
			path: metas[0]?.path ?? "",
		...
		await writeJson(join(targetDir, "project.json"), mergedMeta);
		await writeJson(join(targetDir, "log.json"), log.slice(-config.maxLogEntries));
		await writeJson(join(targetDir, "notes.json"), notes.slice(-config.maxLogEntries));
		if (digestFrom !== void 0) {
			const digest = await readFile(join(digestFrom, "digest.txt"), "utf8").catch(() => void 0);
			if (digest !== void 0 && digest.trim().length > 0) await writeText(join(targetDir, "digest.txt"), digest);
		}
		await renderProgress({ key: mergedMeta.id, dir: targetDir, meta: mergedMeta, metaPath: join(targetDir, "project.json"), cwd: mergedMeta.path, title: mergedMeta.title }, log.slice(-config.maxLogEntries), config);
		return true;
	} catch (error) {
		return false;
	}
...
function apply(ctx, config) {
	...
	// 启动迁移：把旧版本按「会话标题」拆分的同工作区目录合并成一个（非阻塞、幂等）。
	migrateProjects(resolved.projectsDir, resolved).then((count) => {
		if (count > 0) ctx.logger.info(`project-progress: 已合并 ${count} 个同工作区项目目录`);
	}).catch((error) => {
		ctx.logger.warn(`project-progress: 项目目录迁移失败: %o`, error);
	});
	...
	*/
	...
	ctx.commands.register({
		name: "project",
		description: "查看当前工作区的项目进展；`/project sync` 立即刷新摘要，`/project path` 输出进展文件路径，`/project backfill` 为历史会话补建项目文件，`/project merge` 合并同工作区的旧项目目录",
		input: {
			hint: "[sync|path|backfill|merge]"
		},
			...
			if (raw === "backfill") {
				...
				return {
					...
					text: [
						...
						"",
						"用法：/project [sync|path|backfill|merge]"
					].join("\n")
				};
			}
			if (raw === "merge") {
				const count = await migrateProjects(resolved.projectsDir, resolved);
				return {
					kind: "success",
					text: count > 0
						? `已合并 ${count} 组同工作区项目目录（多余目录已删除，数据已归并到按路径命名的目录）。\n用法：/project [sync|path|backfill|merge]`
						: "没有需要合并的目录（每个工作区已只有一个项目目录）。\n用法：/project [sync|path|backfill|merge]"
				};
			}
